...
class Agent:
    ...
    def __init__(
        ...
        *,
        ...
        self._miosa: Any | None = None
        if tools is not None:
            self.tools = list(tools)
        elif miosa_api_key is not None:
            try:
                miosa_module = import_module("miosa")
            except ImportError as exc:
                raise ImportError(
                    "Using miosa_api_key requires the MIOSA Python SDK. "
                    "Install it before using the default MIOSA tool catalogue."
                ) from exc
            miosa_class = getattr(miosa_module, "Miosa")
            if not callable(miosa_class):
                raise TypeError("MIOSA Python SDK does not expose a callable Miosa client.")
            self._miosa = miosa_class(api_key=miosa_api_key, base_url=miosa_base_url)
